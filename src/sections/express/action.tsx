...
import UpdateExpress from './update_express';
...
const ActionButton = ({ row }: any) => {
  ...
  const deleteExpress = () => {
    const prom = APIService.deleteExpress(row?.id ?? row?._id);
    ...
    toast.promise(prom, {
      ...
      success: {
        render({ data }) {
          ...
          const res = data?.data?.message || 'Express deleted successfully';
          setOpenDelete(false);
          return `${res}`;
        },
      },
  ...
  return (
    <>
      <CustomizedDialog
        ...
        title={title}
        body={<UpdateExpress setOpen={setOpenUpdate} data={row} />}
      />
      <CustomizedDialog
        open={openDelete}
        ...
        body={
          <RenderConfirmation
            setOpen={setOpenDelete}
            message="Are you sure you want to delete this express?"
          ...
          {profile &&
            ...
            row?.status !== 'deleted' && (
              <>
                <MenuItem
                  onClick={() => {
                    setTitle(`Update Express Delivery Charge`);
                    handleClosePopover();
                    setOpenUpdate(true);
                  }}
                  sx={{ color: 'info.main' }}
                ...
                <MenuItem
                  onClick={() => {
                    setTitle(`Delete Express Delivery`);
                    handleClosePopover();
                    setOpenDelete(true);
                  }}
                  sx={{ color: 'error.main' }}
